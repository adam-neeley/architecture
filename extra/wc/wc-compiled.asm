...
#    a0 = a character
# on exit:
#    v0 = nonzero if the character was white space, zero otherwise

is_space:
        sll     $a0,$a0,24
        sra     $a0,$a0,24        # sign-extend the character
        addiu   $v1,$a0,-9        # subtract ASCII code for tab (\t)
        andi    $v1,$v1,0x00ff    # mask off all but ASCII
        # the whitespace chars are 9, 10, 11, 12, 13, and 32, so...
        sltiu    $v1,$v1,5        # v1 = 1 iff char was in 9..13
        bne     $v1,$zero,L3      # return true if char was in 9..13
        xori    $a0,$a0,0x20      # toggle bit 5, so if char was 32 (' '),
                                  # a0 will be zero
        sltiu    $v0,$a0,1        # if so, that's whitespace (true),
                                  # otherwise not whitespace (false)
        jr      $ra               # return
...
wc_impl:
        addiu   $sp,$sp,-48        # allocate stack frame
        sw      $ra,44($sp)        # save caller's regs
        sw      $s3,40($sp)
        sw      $s2,36($sp)
        sw      $s1,32($sp)
        sw      $s0,28($sp)
        move    $s1,$a0            # save pointer to string in s1
        move    $s0,$a1            # save pointer to WcResult in s0
        sw      $zero,0($a1)        # zero out all fields in the WcResult
        sw      $zero,4($a1)
        sw      $zero,8($a1)
        move    $s2,$zero        # s2 = inside_word, initialize to false
        li      $s3,10                  # s3 = newline (\n)
        b       L5            # get started

L9:
    # when we get here, a0 = the current char in the string
        lw      $v0,8($s0)        # v0 = num_bytes
        addiu   $v0,$v0,1        # num_bytes++
        sw      $v0,8($s0)        # save it back to the WcResult
        bne     $a0,$s3,L6        # is current char a newline?
                                # fall through: yes
        lw      $v0,0($s0)        # v0 = num_lines
        addiu   $v0,$v0,1        # num_lines++
        sw      $v0,0($s0)        # save it back to the WcResult
L6:
        beq     $s2,$zero,L7        # are we inside a word?
                    # fall through: yes
        jal     is_space
        beq     $v0,$zero,L5        # is current char whitespace?
                    # fall through: yes
        move    $s2,$zero        # then we're no longer inside a word
        b       L5            # go to next character
L7:
        jal     is_space
        bne     $v0,$zero,L5        # is current char whitespace?
                    # fall through: no
        lw      $v0,4($s0)        # v0 = num_words
        addiu   $v0,$v0,1        # num_words++
        sw      $v0,4($s0)        # save it back to the WcResult
        li      $s2,1            # we're inside a word
L5:
        addiu   $s1,$s1,1        # s1 -> next char after current char
        lb      $a0,-1($s1)        # a0 = current char
        bne     $a0,$zero,L9        # if not end of string, keep going

        lw      $ra,44($sp)        # restore caller's regs
        lw      $s3,40($sp)
        lw      $s2,36($sp)
        lw      $s1,32($sp)
        lw      $s0,28($sp)
